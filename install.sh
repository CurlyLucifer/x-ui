...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/CurlyLucifer/x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        echo -e "The latest version of x-ui was detected:${last_version}，start installation"
        ...
        last_version=$1
        url="https://github.com/CurlyLucifer/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
    ...
    #echo -e "Please make sure that this port is not occupied by other programs，${yellow}And make sure port 54321 is released${plain}"
